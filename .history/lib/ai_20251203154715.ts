...
export async function generateStudyPlan(
  ...
  // Use Gemini 1.5 Flash for its speed and large context window (1M tokens)
  // This allows us to process entire textbooks or large PDFs in one go,
  // which is superior to standard RAG (chunking) for "whole document" planning.
  ...
  try {
    ...
    // With responseMimeType: "application/json", the model should return valid JSON.
    // We still wrap in try/catch just in case.
    ...
    if (error.response) {
      console.error("Gemini Response Error Details:", JSON.stringify(error.response, null, 2));
    }
    ...
    try {
      const groqCompletion = await groq.chat.completions.create({
        messages: [
          ...
          },
          {
            ...
            Create a study plan for this content (truncated for length):
            ${content.substring(0, 25000)}` // Groq has smaller context limit
          }
        ],
      ...
      throw new Error("Failed to generate study plan with both Gemini and Groq.");
    }
  }
}
